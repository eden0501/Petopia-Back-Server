...
describe("Post API", () => {
    ...
    test("POST /posts - Create a new post", async () => {
        ...
        for (const post of postsData) {
            response = await request(app).post("/posts")
    ...
    test("GET /posts - Get posts by filter (type)", async () => {
        const responseOther = await request(app).get(`/posts?type=${postsData[0].type}`)
            .set("Authorization", "Bearer " + userData.accessToken);
        ...
        const responseDonation = await request(app).get(`/posts?type=${postsData[1].type}`)
            .set("Authorization", "Bearer " + userData.accessToken);
