...
describe("Chat API", () => {
    ...
    describe("POST /chat with user context", () => {
        ...
        beforeAll(async () => {
            // Create posts and comments so the context-building code is exercised
            const postRes = await request(app)
                .post("/posts")
                .set("Cookie", [`accessToken=${userData.accessToken}`])
                .send({ title: "My Dog", content: "He is great", type: PostTypes.OTHER });
            ...
            // Create a second user with posts for community pulse
            const uniqueSuffix = Date.now() + "_" + Math.random().toString(36).slice(2, 7);
            const secondUser = { username: "chatCtx_" + uniqueSuffix, email: "chatCtx_" + uniqueSuffix + "@test.com", password: "password123" };
            ...
            const post2Res = await request(app)
                ...
                .send({ title: "Community Post", content: "Hello everyone", type: PostTypes.KNOWLEDGE });
